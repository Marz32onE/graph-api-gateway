...
- **NEW** OpenAPI 3.1 docs pipeline: `swag` annotations on handlers → `go tool swag init` generates the `docs` package (the spec compiled into the binary as a Go string) → served at `/openapi.json` via `docs.SwaggerInfo.ReadDoc()` (no file embedding; swag v2's stray `schemes` field stripped), with an **offline Swagger UI** (swaggo/files, all assets in-binary) at `/docs/` pointed at `/openapi.json`. CI gate via `make check-docs`.
