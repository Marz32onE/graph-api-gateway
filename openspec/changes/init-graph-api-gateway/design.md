...
- OpenAPI 3.1 spec **generated from `swag` annotations on handlers** (the kube-state-graph pattern), postprocessed to lift parameter `example` into `schema.example`, copied into `internal/api/static/openapi/`, served at `/openapi.yaml` + `/openapi.json`, with an offline **Swagger UI** served at `/docs/*filepath` from the embedded `github.com/swaggo/files/v2` bundle.
...
│   │   ├── docs.go                    # /openapi.{yaml,json} (with swag annotations)
...
├── tools/openapi-postprocess/         # rewrites parameter `example` → `schema.example`
│   └── main.go
...
**Why**: this is the `cmd/` + `internal/` layout endorsed by [golang-standards/project-layout](https://github.com/golang-standards/project-layout) and `kube-state-graph` already follows it. Keeping `internal/api/static/openapi/` embedded (via `//go:embed`) lets us ship a single binary with no runtime asset dependency; the Swagger UI assets are embedded directly from `github.com/swaggo/files/v2`. Handlers carry `swag` annotations and are the source of truth; `make docs` regenerates `docs/openapi.{yaml,json}` and copies them into `internal/api/static/openapi/` for embedding — identical pipeline to kube-state-graph.
...
### D7. OpenAPI documentation — swag annotations + postprocess + offline Swagger UI (mirror kube-state-graph)
...
3. **Postprocess**: `go run ./tools/openapi-postprocess docs/swagger.json docs/swagger.yaml` — port the same tool (mutates parameters so `parameter.example` is also written under `parameter.schema.example`, smoothing parameter examples for query/path params).
4. **Embed**: `make docs` then copies `docs/swagger.{yaml,json}` → `internal/api/static/openapi/openapi.{yaml,json}` (embedded via `//go:embed static/openapi/*`). The Swagger UI assets are embedded separately, directly from `github.com/swaggo/files/v2`.
5. **Serve**:
   - `GET /openapi.yaml` → embedded YAML, `Content-Type: application/yaml` (`handleOpenAPIYAML`)
   - `GET /openapi.json` → embedded JSON, `Content-Type: application/json` (`handleOpenAPIJSON`) — the UI fetches this at runtime
...
6. **UI**: **offline Swagger UI** 5.18.2, served from the embedded `github.com/swaggo/files/v2` (`v2.0.2`) bundle (which exports `var FS` embedding `dist/*`) — no network fetch, no vendored bundle to refresh.
...
- **Open paths**: `/livez`, `/readyz`, `/openapi.yaml`, `/openapi.json`, `/docs/*filepath` (gin route patterns matched via `c.FullPath()`).
