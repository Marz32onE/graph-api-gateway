...
- [x] 2.1 Implement `internal/config/config.go` defining `Config` struct with fields for listen addr, log level/format, primary/switch backend URL+API key+timeout
...
- [x] 2.3 Implement startup validation: both `BACKEND_PRIMARY_URL` and `BACKEND_SWITCH_URL` required and must parse as valid URLs with scheme; fail with clear error naming the missing/invalid key
...
- [x] 4.3 Implement `internal/client/kubestategraph.go` (`KubeStateGraphClient` struct): constructor takes base URL, API key, timeout; uses a `*resty.Client` built from the otel-instrumented http client; `Name()` returns `"primary"`; `FetchGraph` issues `GET <baseURL>/v1/graph?<rawQuery>`, sets `X-API-Key` header when configured, decodes into `CytoscapeGraph`
- [x] 4.4 Rename `internal/client/secondary.go` → `internal/client/switch.go`; rename type `SecondaryGraphClient` → `SwitchGraphClient` and constructor accordingly; `Name()` returns `"switch"`
- [x] 4.5 Compile-time interface assertions: `var _ GraphBackend = (*KubeStateGraphClient)(nil)` and same for `SwitchGraphClient`
- [x] 4.6 Unit test against `httptest.Server`: API key forwarded / omitted, valid payload decodes, **`ipaddress` array decodes onto `NodeData`** (and missing `ipaddress` decodes as nil), malformed JSON returns wrapped error, missing `elements` decodes as empty slices, timeout triggers deadline-exceeded error, `traceparent` propagated when a parent span exists, `SwitchGraphClient` correctly forwards an `?ip=…&ip=…` query verbatim
...
- [x] 6.3 Register middleware order: `gin.Recovery()` → `requestIDMiddleware` → `otelgin.Middleware("graph-api-gateway")` → `loggingMiddleware`
- [x] 6.4 Implement `internal/api/health.go`: `GET /livez` and `GET /readyz` both return `200` with body `ok`
- [x] 6.5 **Rewrite** `internal/api/handlers.go` `GET /v1/graph` as a sequential pipeline: (1) call primary with `context.WithTimeout(cfg.Primary.Timeout)` forwarding `c.Request.URL.RawQuery`; (2) `ips := pipeline.ExtractIPs(primary)` (node-only); (3) if `len(ips) > 0`, call switch with `context.WithTimeout(cfg.Switch.Timeout)` using `buildIPQuery(ips)`; (4) `reconciled := pipeline.ReconcileSwitch(primary, switchOrNil)`; (5) `merge.Merge(primary, reconciled)`; (6) on any stage error → `502 {"error":"upstream_unavailable"}`
...
- [x] 10.5 Add `internal/api/handlers.go` helper `buildIPQuery(ips []string) string` using `url.Values{"ip": ips}.Encode()`; unit test that two IPs round-trip through `url.ParseQuery` to `{"ip":["10.0.0.1","10.0.0.2"]}`
- [x] 10.6 Rename `internal/client/secondary.go` → `internal/client/switch.go`; rename struct/constructor/`Name()` from `secondary` to `switch`; update package-level docs
- [x] 10.7 Rename env keys: `BACKEND_SECONDARY_URL` → `BACKEND_SWITCH_URL`, `BACKEND_SECONDARY_API_KEY` → `BACKEND_SWITCH_API_KEY`, `BACKEND_SECONDARY_TIMEOUT` → `BACKEND_SWITCH_TIMEOUT`; rename `Config.Secondary` → `Config.Switch`; update `config_test.go` table cases
